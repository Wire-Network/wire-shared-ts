...
export class SigningRequest {
  ...
  private static identityAbi(version: number) {
    ...
    abi.actions = [
      { name: "identity", type: "identity", ricardian_contract: "" }
    ]
    return abi
  }
  ...
  private static identityType(
    version: number
  ): typeof IdentityV2 | typeof IdentityV3 {
  ...
  private static storageType(
    version: number
  ): typeof RequestDataV3 | typeof RequestDataV2 {
  ...
  public static createSync(
    ...
    if (args.chainId === null) {
      ...
      data.chain_id = ChainId.from(args.chainId || ChainName.SYS).chainVariant
    }
    ...
    if (args.chainIds && args.chainId === null) {
      ...
      data.info.push({
        ...
        value: Serializer.encode({
          object: ids,
          type: { type: ChainIdVariant, array: true }
        })
      })
    }
  ...
  public encode(
    ...
    const shouldCompress =
      compress !== undefined ? compress : this.zlib !== undefined
  ...
  public resolveActions(
    ...
    return this.getRawActions().map(rawAction => {
      ...
      if (isIdentity(rawAction)) {
        abi = (this.constructor as typeof SigningRequest).identityAbi(
          this.version
        )
      } else {
  ...
  public resolveTransaction(
    ...
    const context_free_actions =
      tx.context_free_actions as unknown as ResolvedAction[]
  ...
  public getIdentityPermission(): Name | null {
    ...
    if (
      id.permission &&
      !id.permission.permission.equals(PlaceholderPermission)
    ) {
  ...
  public setInfoKey(
    ...
    if (typeof object === "string" && !type) {
      ...
      // sysio-abi encoded strings (varuint32 length prefix + utf8 bytes)
  ...
  public getInfoKey<T extends ABISerializableConstructor>(
    key: string,
    type: T
  ): InstanceType<T>
  ...
  public clone(): SigningRequest {
    ...
    if (this.signature) {
      signature = RequestSignature.from(
        JSON.parse(JSON.stringify(this.signature))
      )
    }

    const RequestData = (this.constructor as typeof SigningRequest).storageType(
      this.version
    )
...
export class IdentityProof extends Struct {
  @Struct.field(ChainId) declare chainId: ChainId
  @Struct.field(Name) declare scope: Name
  @Struct.field(TimePointSec) declare expiration: TimePointSec
  @Struct.field(PermissionLevel) declare signer: PermissionLevel
   ...
   */
  recover(): PublicKey {
    return this.signature.recoverDigest(
      this.transaction.signingDigest(this.chainId).msgDigest
    )
  }
