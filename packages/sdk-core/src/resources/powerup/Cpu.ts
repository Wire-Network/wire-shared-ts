...
export class PowerUpStateResourceCPU extends PowerUpStateResource {
  ...
  frac_by_ms = (usage: SampleUsage, ms: number) =>
    this.frac_by_us(usage, ms * 1000)
  ...
  ): number => this.price_per_us(usage, us, options)
  ...
  price_per_ms = (
    ...
    ms = 1,
    options?: PowerUpStateOptions
  ): number => this.price_per_us(usage, ms * 1000, options)
