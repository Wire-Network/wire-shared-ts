...
export type BlockIdType =
  | BlockId
  | BytesType
  | { blockNum: UInt32Type; checksum: Checksum256Type }

export class BlockId implements ABISerializableObject {
  static abiName = "block_id_type" // sysio contract context defines this with a _type suffix for some reason
  ...
  constructor(array: Uint8Array) {
    if (array.byteLength !== 32) {
      throw new Error(
        `BlockId size mismatch, expected 32 bytes got ${array.byteLength}`
      )
    }
