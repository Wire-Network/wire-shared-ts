...
export class Variant implements ABISerializableObject {
  ...
  constructor(variant: [string, ABISerializable]) {
    ...
    const variantIdx = abiVariant
      .map(abiTypeString)
      .findIndex(t => t === variant[0])
  ...
  equals(other: AnyVariant): boolean {
    ...
    return abiEncode({ object: this }).equals(
      abiEncode({ object: otherVariant })
    )
  }

  get variantName(): string {
    const variant = (this.constructor as VariantConstructor).abiVariant![
      this.variantIdx
    ]
