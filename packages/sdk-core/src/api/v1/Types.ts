...
export class AccountVoterInfo extends Struct {
  ...
  @Struct.field("uint32") declare reserved2: UInt32
...
export class AccountRexInfoMaturities extends Struct {
  /** Expected results from after SYSIO.Contracts v1.9.0 */
  @Struct.field("time_point", { optional: true }) key?: TimePoint
  @Struct.field("int64", { optional: true }) value?: Int64
  /** Expected results from before SYSIO.Contracts v1.9.0 */
...
export class AccountObject extends Struct {
  ...
  getPermission(permission: NameType): AccountPermission {
    ...
    if (!match) {
      throw new Error(
        `Unknown permission ${name} on account ${this.account_name}.`
      )
    }
  ...
  get transaction(): Transaction | undefined {
    if (this.extra.packed_trx) {
      switch (this.extra.compression) {
        ...
        default: {
          throw new Error(
            `Unsupported compression type ${this.extra.compression}`
          )
        }
      }
    }
...
export class TransactionTraceReceipt extends Struct {
  ...
  declare auth_sequence: TransactionAuthSequence[]
...
export class TransactionTrace extends Struct {
  ...
  declare receipt: TransactionTraceReceipt
