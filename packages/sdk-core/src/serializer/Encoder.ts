...
export function abiEncode(args: EncodeArgs): Bytes {
  ...
  if (typeof args.type === "string") {
    ...
    args.type &&
    (args.type as ABISerializableConstructor).abiName !== undefined
  ) {
    type = args.type as ABISerializableConstructor
    typeName = (args.type as ABISerializableConstructor).abiName
  } else {
...
export function encodeAny(
  ...
  function encodeInner(value: any) {
    ...
    if (abiType && abiType.toABI) {
      ...
      typeof value.toABI === "function" &&
      value.constructor.abiName === type.name
    ) {
      ...
      if (type.fields) {
        ...
        if (typeof value === "string") {
          ...
          if (ev === undefined) {
            const names = enumInfo.values.map(v => `'${v.name}'`).join(", ")
            throw new Error(
              `Unknown enum value '${value}' for '${type.name}', expected one of ${names}`
            )
          }

          intValue = ev.value
        } else {
          intValue = typeof value === "number" ? value : Number(value)
        }

        // Encode as the underlying integer type
        encodeAny(intValue, new ABI.ResolvedType(enumInfo.type), ctx)
        ...
        if (vIdx === -1) {
          ...
          throw new Error(
            `Unknown variant type '${vName}', expected one of ${types}`
          )
        }
        ...
        if (!abiType) {
          throw new Error(
            type.name === "any"
              ? "Unable to encode any type to binary"
              : "Unknown type"
          )
        }
        ...
        if (!instance.toABI) {
          throw new Error(
            `Invalid type ${type.name}, no encoding methods implemented`
          )
        }
