...
export type ABISerializableType =
  | string
...
export function synthesizeABI(type: ABISerializableConstructor) {
  ...
  const resolve = (t: ABISerializableConstructor) => {
    if (!t.abiName) {
      ...
      throw new Error(
        "Misconfigured Struct subclass, did you forget @Struct.type?"
      )
    }
...
export function isTypeDescriptor(
  type: ABISerializableType
): type is ABITypeDescriptor {
  return (
    typeof type !== "string" &&
...
export function isABISerializableObject(
  value: any
): value is ABISerializableObject {
  return (
    value && value.constructor && typeof value.constructor.abiName === "string"
  )
}
